...
const TOKENS = [
  ...
  { symbol: "USDT", color: "bg-emerald-500", balance: "3,100.00" },
...
const POOL = {
  ...
  composition: [
    ...
    { token: "USDT", amount: "$4,724,640", pct: 38.1, color: "bg-emerald-500" },
    ...
    <div className="min-h-screen bg-[#08080f]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          {/* ── Left panel: Pool overview + Composition ─────────────────── */}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-violet-900/10">
                  {["Type", "From", "To", "Time", "Wallet"].map((h) => (
                    <th
                      key={h}
                      className="px-6 py-3 text-left text-xs text-gray-600 font-medium whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {RECENT_TXS.map((tx, i) => (
                  <tr
                    key={i}
                    className="border-b border-violet-900/10 hover:bg-white/[0.02] transition-colors"
                  >
...
function SwapTab({
  ...
  const toAmount = fromAmount
    ? (parseFloat(fromAmount) * 0.9999).toFixed(2)
    : "";
  const fee = fromAmount
    ? (parseFloat(fromAmount) * 0.0001).toFixed(4)
    : "0.0000";
  const minReceived = toAmount
    ? (parseFloat(toAmount) * 0.999).toFixed(2)
    : "0.00";
    ...
    <div>
      ...
      <div className="bg-white/5 rounded-xl p-4 mb-2">
        ...
        {walletConnected && fromAmount === "" && (
          <button
            onClick={() =>
              setFromAmount(fromBal.replace(/,/g, ""))
            }
...
function DepositTab({
  ...
  const poolShare =
    total > 0
      ? ((total / 12_400_000) * 100).toFixed(5)
      : "0.00000";

  return (
    <div>
      <p className="text-sm text-gray-400 mb-5">
        Deposit any combination of stablecoins. You receive SBLP tokens
      ...
      {TOKENS.map((token, i) => (
        <div key={token.symbol} className="bg-white/5 rounded-xl p-4 mb-3">
          ...
          <div className="flex items-center gap-3">
            <div
              className={`w-8 h-8 rounded-full ${token.color} flex items-center justify-center text-xs font-bold text-white flex-shrink-0`}
            >
              {token.symbol[0]}
            </div>
            <input
...
function WithdrawTab({
  ...
  const usdt = (amount * 0.381).toFixed(2);
  const eurc = (amount * 0.196).toFixed(2);

  return (
    <div>
      <p className="text-sm text-gray-400 mb-5">
        Burn SBLP tokens to receive USDC, USDT, and EURC proportional to the
        pool composition.
      </p>
        ...
        <>
          ...
          <div className="space-y-2 mb-4">
            {[
              ...
              { token: "USDT", amount: usdt, color: "bg-emerald-500" },
              ...
              >
                <div className="flex items-center gap-2">
                  <div
                    className={`w-6 h-6 rounded-full ${item.color} flex items-center justify-center text-xs font-bold text-white`}
                  >
                    {item.token[0]}
                  </div>
